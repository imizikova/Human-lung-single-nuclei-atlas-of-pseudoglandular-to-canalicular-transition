---
title: "Subclustering_endothelium"
...
```{r}
cells_keep <- colnames(lungs)[lungs$seurat_clusters %in% c(4,6)]
Endothelial_subset <- subset(lungs, cells=cells_keep)
```


#Integrate data
```{r}
seurat.list <- SplitObject(Endothelial_subset, split.by="GA")
...
```{r}
saveRDS(seurat.integrated, "C:/.../Endothelial.rds")
```
